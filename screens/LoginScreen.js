...
import { postSignIn } from "../utils/request";
...
const LoginScreen = () => {
  ...
  const loginPressHandler = async () => {
    const loginData = await postSignIn({ email, password });
    dispatch(login({ token: loginData.token, user: loginData.data.user }));
  };
  ...
  return (
    ...
    <LinearGradient  colors={['#4AAAEC', 'rgba(169, 220, 255, 0)']} style={styles.container}>
      ...
      <Text style={styles.formTitle}>Bạn đã có tài khoản với Meow?</Text>
      <Image
      ...
      <View style={styles.optionContainer}>
        <Pressable onPress={forgotPasswordPressHandler} >
          <Text style={styles.buttonRegisterText}>Quên mật khẩu ?</Text>
        </Pressable>
      </View>
